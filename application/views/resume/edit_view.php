...
<p class="fs-2">Редагування резюме</p>
<div>
    <form method="post" enctype="multipart/form-data" style="margin-bottom: 20px">
        <div class="mb-4">
            <label for="exampleInput1" class="form-label">Професія, посада на яку ви претендуєте</label>
            <input type="text" required name="job_pos" value="<?= $data[0]['job_pos']?>" class="form-control" id="exampleInput1">
        </div>
        <div class="mb-4">
            <label for="exampleInput2" class="form-label">Ім'я</label>
            <input type="text" required name="first_name" value="<?= $data[0]['first_name']?>" class="form-control" id="exampleInput2">
        </div>
        <div class="mb-4">
            <label for="exampleInput2" class="form-label">Прізвище</label>
            <input type="text" required name="last_name" value="<?= $data[0]['last_name']?>" class="form-control" id="exampleInput2">
        </div>
        <div class="mb-4">
            <label for="exampleInput2" class="form-label">Ваш вік</label>
            <input type="number" min="18" max="120" value="<?= $data[0]['age']?>" required name="age" class="form-control" id="exampleInput2">
        </div>
        <div class="mb-4">
            <label for="email" class="form-label">Електронна адреса</label>
            <input type="email" name="email" class="form-control" id="email" value="<?= $data[0]['email']?>" required pattern="[a-zA-Z0-9.-_]{1,}@[a-zA-Z.-]{2,}[.]{1}[a-zA-Z]{2,}" aria-describedby="emailHelp">
        </div>
        <div class="mb-4">
            <label for="email" class="form-label">Місто, в якому ви хочете працювати</label>
            <?php $cities = array($data[0]['city'],'Київ','Житомир', 'Харків','Дніпро','Одеса','Донецьк','Запоріжжя','Львів','Кривий Ріг','Миколаїв','Маріуполь','Луганськ','Макіївка','Вінниця','Сімферополь','Севастополь','Херсон','Полтава','Чернігів','Черкаси','Суми','Горлівка','Хмельницький','Камянське','Кропивницький','Рівне','Чернівці','Кременчук','Тернопіль','Івано-Франківськ','Луцьк','Біла Церква','Краматорськ','Мелітополь','Керч','Нікополь','Словянськ','Бердянськ','Ужгород','Умань','Бердичів','Шостка','Бровари','Новоград-Волинський');?>
        ...
        <div class="input-group mb-4">
            ...
            <input required type="text" value="<?= $data[0]['salary_exp']?>" name="salary_exp" class="form-control" id="salary">
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <?php $emp = array($data[0]['employment'], 'повна зайнятість','стажування / практика', 'неповна зайнятість','віддалена робота','проектна робота','позмінна робота');?>
            <select name="employment" required id="employment" class="form-select">
                <?php
                foreach($emp as $r){
                    echo '<option value="'.$r.'">'.$r.'</option>';
                }
        ...
        <div class="input-group mb-4">
            ...
            <input class="form-control" value="<?= $data[0]['contacts']?>" required type="tel" id="phone" name="contacts" placeholder="000-00-00-000" pattern="[0-9]{3}[0-9]{2}[0-9]{2}[0-9]{3}">
        </div>
        <div class="mb-4">
            <label class="form-label">Деталі про досвід роботи:</label>
            <textarea name="descriptions" id="default">"<?= $data[0]['descriptions']?>"</textarea>
        </div>
        <div class="mb-4">
            <?php if(is_file('images/resumes/'.$data[0]['photo'])): ?>
                <img alt="" src='/images/resumes/<?= $data[0]['photo']?>' />
            <?php endif; ?>
        </div>
        <div class="mb-4">
        ...
        <button type="submit" name="submit" id="btn" class="btn btn-primary">Змінити</button>
    </form>
</div>
...
<script>
    ...
    alert(<?= $data[0]['city'] ?>)
    document.getElementById('job_exp_rubric').value = <?= $data[0]['job_exp_rubric'] ?>
